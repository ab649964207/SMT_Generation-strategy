...
#         # print("###################",action.get_all_params(var_dict)) #(['?k', '?l'], {(1,), (1, 1)}, True)
...
#                         # print("eeeeeeeeeeeeeeeeeeee",param_dict)  #{'?k': 1}
#
...
#             ###########################
#             elif param_set is not None and len(param_set) > 1:
    ...
    def gen_eff(self, state):  #将状态放进去，state再代入v中 ，得到的是后续状态
        ...
        for action in self.domain.actions:
            ...
            if ok:
                if  param_set is not None and len(param_set)  == 1 :
                    # print("$$$$$$$$$$$$$$$$$$$",param)
                    if param_set[0] is not None and len(param_set[0]) > 0:
                        for k in param_set[0]:
                            param_dict.append({param[0]: k})
                            # print("eeeeeeeeeeeeeeeeeeee",param_dict)  #{'?k': 1}
